      ...
      {viewingReport && (
        <Modal title={viewingReport.title} onClose={() => setViewingReport(null)}>
          <div className="flex flex-col gap-4 max-h-[80vh] overflow-y-auto">
            <div style={{ background: "#FFFFFF", border: "1px solid #E5E5EA" }} className="rounded-lg p-3 grid grid-cols-2 gap-3 text-xs">
              <div className="text-[#86868B]">Academic Year: <span className="text-white font-medium">{viewingReport.year}</span></div>
              <div className="text-[#86868B]">Semester: <span className="text-white font-medium">Sem {viewingReport.sem}</span></div>
              <div className="text-[#86868B]">Date Generated: <span className="text-white font-medium">{viewingReport.date}</span></div>
              <div className="text-[#86868B]">Generated By: <span className="text-white font-medium">{viewingReport.by}</span></div>
            </div>
      ...
      {modal && (
        <Modal title="Create User" onClose={() => setModal(false)}>
          ...
          <div className="flex flex-col gap-3">
            ...
            <div style={{ background: "#F5F5F7", border: "1px solid #E5E5EA" }} className="rounded-lg p-3 text-xs text-[#86868B]">
    ...
    <div>
      ...
      <div className="relative mb-6">
        ...
        <input value={q} onChange={e => setQ(e.target.value)} placeholder="Search students…"
          style={{ background: "#FFFFFF", border: "1px solid #E5E5EA", color: "#E2E8F0", fontSize: 16 }}
      ...
      {q.length <= 1 && (
        <div className="text-center py-20 text-slate-600">
          <Icon d={icons.search} size={48} color="#E5E5EA" />
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div style={{ background: "#FFFFFF", border: "1px solid #E5E5EA" }} className="rounded-xl p-5">
...
export function renderConsolidationTables(condData) {
  ...
  const arrearRows = arrearNames.map((name, i) => [
    name,
    <span className="font-bold text-[#1D1D1F]">{arrearCounts[i] || 0}</span>
  ]);
  ...
  const deptRows = departments.map((d, idx) => [
    idx + 1,
    <span className="font-bold text-[#424245]">{d.name}</span>,
    d.strength,
    d.appeared,
    d.absent,
  ...
  const totalRow = [
    ...
    <span className="font-bold text-[#86868B]">{totalArrears[0]}</span>,
    <span className="font-bold text-[#86868B]">{totalArrears[1]}</span>,
    <span className="font-bold text-[#86868B]">{totalArrears[2]}</span>,
    <span className="font-bold text-[#86868B]">{totalArrears[3]}</span>,
    <span className="font-bold text-[#86868B]">{totalArrears[4]}</span>,
    <span className="font-bold text-[#86868B]">{totalArrears[5]}</span>,
    <span className="font-bold text-[#86868B]">{totalArrears[6]}</span>,
  ...
  return (
    <div className="flex flex-col gap-6">
      {/* Table 1: Overall Summary */}
      <div style={{ background: "#FFFFFF", border: "1px solid #E5E5EA" }} className="rounded-xl p-5 overflow-x-auto">
      ...
      <div style={{ background: "#FFFFFF", border: "1px solid #E5E5EA" }} className="rounded-xl p-5 max-w-md overflow-x-auto">
...
const ProgressBar = ({ label, value, max = 100, color = "#0071E3", subLabel }) => {
  const pct = Math.round((value / max) * 100) || 0;
  return (
    <div className="mb-4">
      <div className="flex justify-between items-center mb-1 text-xs">
        <span className="text-[#424245] font-medium">{label}</span>
        <span className="text-[#86868B] font-semibold">{value} {subLabel || `(${pct}%)`}</span>
      </div>
      <div className="w-full h-2.5 bg-white rounded-full overflow-hidden border border-[#E5E5EA]/30">
...
const StatCard = ({ label, value, sub, icon, accent = "#0071E3" }) => (
  <div style={{ background: "#FFFFFF", border: "1px solid #E5E5EA" }} className="rounded-xl p-4 flex flex-col gap-1">
    <div className="flex items-center justify-between mb-1">
      <span className="text-[#86868B] text-xs font-medium">{label}</span>
...
export function ResultPage() {
  ...
  const ModeToggle = () => (
    <div className="flex gap-0.5 p-1 rounded-xl" style={{ background: "#E5E5EA", display: "inline-flex" }}>
      {[
        ...
        <button
          ...
          style={{
            background: uploadMode === m.id ? "#0071E3" : "transparent",
      ...
      <div>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
          <div>
            ...
            <p className="text-[#86868B] text-sm mt-1">Upload and analyse semester results by department</p>
          </div>
          <ModeToggle />
        </div>
        <MultiDeptResultPage
    ...
    return (
      <div>
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          <div>
            <h1 className="text-2xl font-bold text-white">Result Analysis</h1>
            <p className="text-[#86868B] text-sm mt-1">{analysisData.title} · {analysisData.year} (Sem {analysisData.sem})</p>
          </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          <StatCard label="Total Students" value={condData.overall.totalStudents} sub="Overall Strength" icon="students" accent="#0071E3" />
          <StatCard label="Present Students" value={condData.overall.appeared} sub="Attended Exams" icon="check" accent="#34C759" />
          <StatCard label="Absent Students" value={condData.overall.absent} sub="Absent from Exams" icon="close" accent="#FF3B30" />
          <StatCard label="Pass Rate" value={`${condData.overall.passPercent.toFixed(1)}%`} sub={`${condData.overall.passed} passed`} icon="check" accent="#34C759" />
          <StatCard label="Fail Rate" value={`${(100 - condData.overall.passPercent).toFixed(1)}%`} sub={`${condData.overall.failed} failed`} icon="close" accent="#FF3B30" />
          <StatCard label="Average GPA" value={condData.overall.avg} sub="GPA Average" icon="chart" accent="#5E5CE6" />
          <StatCard label="Topper" value={condData.overall.topper} sub="Top Performer" icon="students" accent="#FF9500" />
        </div>
        ...
        <div style={{ background: "#FFFFFF", border: "1px solid #E5E5EA" }} className="rounded-xl p-5 mb-4">
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-white">Result Analysis</h1>
          <p className="text-[#86868B] text-sm mt-1">Upload semester result Excel and generate floor-wise reports</p>
        </div>
        <ModeToggle />
      </div>
          ...
          <div key={i} className="flex items-center">
            <div className="flex items-center gap-2">
              <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0"
                style={{ background: step >= i ? "#0071E3" : "#E5E5EA", color: step >= i ? "white" : "#475569", border: step >= i ? "none" : "1px solid #E5E5EA" }}>
                {step > i ? <Icon d={icons.check} size={12} /> : i + 1}
              </div>
              <span className="text-xs hidden sm:block" style={{ color: step >= i ? "#93C5FD" : "#475569" }}>{s}</span>
            </div>
            {i < 3 && <div className="w-8 sm:w-16 h-px mx-2" style={{ background: step > i ? "#0071E3" : "#E5E5EA" }} />}
          </div>
        ))}
      </div>

      {error && <div style={{ background: "#FF3B3015", border: "1px solid #FF3B3030", color: "#FCA5A5" }} className="rounded-lg p-3 text-sm mb-4">{error}</div>}

      {step === 0 && (
        <div style={{ background: "#FFFFFF", border: "1px solid #E5E5EA" }} className="rounded-xl p-8 text-center">
          <Icon d={icons.upload} size={48} color="#0071E3" />
          <h3 className="text-white font-semibold mt-4 mb-2">Upload Semester Result Excel</h3>
          <p className="text-[#86868B] text-sm mb-2 max-w-md mx-auto">
      ...
      {step === 1 && (
        <div style={{ background: "#FFFFFF", border: "1px solid #E5E5EA" }} className="rounded-xl p-12 text-center">
          ...
          <p className="text-[#86868B] text-sm">Detecting subjects · Matching register numbers · Grouping by floor</p>
        </div>
      )}
          ...
          {parseInfo && (
            <div className="mb-4">
              <div className="grid grid-cols-3 gap-3 mb-2">
                {[
                  { label: "Total Records", value: parseInfo.total, color: "#0071E3" },
                  { label: "Hostellers Matched", value: parseInfo.matched, color: "#34C759" },
                  { label: "Unmatched (skipped)", value: parseInfo.unmatched, color: parseInfo.unmatched > 0 ? "#FF9500" : "#64748B" },
                ].map((it, i) => (
                  <div key={i} style={{ background: "#FFFFFF", border: "1px solid #E5E5EA" }} className="rounded-lg p-3 text-center">
              ...
              {parseInfo.unmatched > 0 && parseInfo.sampleParsed && (
                <div style={{ background: "#FFFBEB", border: "1px solid #FDE68A" }} className="rounded-lg p-3 text-xs">
                  <div className="text-amber-400 font-medium mb-1">Debug — Reg No mismatch check:</div>
                  <div className="text-[#86868B]">From Excel: <span className="font-mono text-amber-300">{parseInfo.sampleParsed.join("  |  ")}</span></div>
                  <div className="text-[#86868B] mt-1">From Student Master: <span className="font-mono text-blue-300">{parseInfo.sampleStudents.join("  |  ")}</span></div>
          ...
          <div style={{ background: "#FFFFFF", border: "1px solid #E5E5EA" }} className="rounded-xl p-5 mb-4">
            ...
            {report.length === 0 ? (
              <div className="text-center py-8 text-[#86868B] text-sm">
                <div className="text-[#86868B] text-sm mb-2">No hostellers matched from the result file.</div>
                <div className="text-slate-500 text-xs">Make sure you have first imported students via <strong className="text-[#424245]">Student Master → Import Excel</strong>, and that the Register Numbers in the result Excel (e.g. 922525243001) exactly match those in the student master.</div>
              </div>
            ) : (
      ...
      {step === 3 && (
        <div style={{ background: "#FFFFFF", border: "1px solid #E5E5EA" }} className="rounded-xl p-12 text-center">
          <div style={{ background: "#34C75915" }} className="rounded-full p-5 inline-flex mb-5">
            <Icon d={icons.check} size={40} color="#34C759" />
          </div>
          <h3 className="text-white font-semibold text-xl mb-2">Report Saved Successfully</h3>
          <p className="text-[#86868B] text-sm mb-6">Available in Report History</p>
